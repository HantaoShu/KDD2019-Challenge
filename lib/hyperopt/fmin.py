...
class FMinIter(object):
    ...
    def run(self, N, time_left, block_until_done=True):
        """
            ...
            with tqdm(total=N+qlen, file=orig_stdout, postfix='best loss: ?',
                ...
                while n_queued < N:
                    ...
                    while qlen < self.max_queue_len and n_queued < N:
                        ...
                        max_time = max(max_time,use_time)
                        if time_left-(time.time()-tot_begin_time) < max_time:
                            stopped = True
                        self.trials.refresh()
                        if 0:
    ...
    def __next__(self):
        self.run(1, 100,block_until_done=self.asynchronous)
        if len(self.trials) >= self.max_evals:
            raise StopIteration()
        return self.trials

    def exhaust(self,time_left):
        n_done = len(self.trials)
        self.run(self.max_evals - n_done, block_until_done=self.asynchronous,time_left=time_left)
        self.trials.refresh()
        return self
